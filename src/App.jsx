...
const NavLink = ({ item, scrollToSection }) => (
  <button
...
const MobileMenu = ({ isMenuOpen, scrollToSection }) => (
  <motion.div
    ...
    className="md:hidden absolute top-16 left-0 right-0 bg-white/90 backdrop-blur-sm shadow-lg"
  >
    <div className="flex flex-col space-y-4 p-6">
      {['home', 'about', 'services', 'why-us', 'testimonials', 'contact'].map((item) => (
        <NavLink key={item} item={item} scrollToSection={scrollToSection} />
      ))}
    </div>
  </motion.div>
);
